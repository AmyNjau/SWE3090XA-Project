...
`android/` is generated by `flutter create` and is deliberately **not tracked in
git** (it holds machine-specific paths in `local.properties`). It is included in
the submitted copy of the project. If you are working from a fresh clone of the
repository, or you need the iOS/web folders, generate them first:

```bash
flutter create .          # generates android/, ios/, web/ without touching lib/
```

and then re-apply the platform configuration below, plus the Firebase wiring:
...
Already applied in the `android/` folder shipped here. After a regenerating
