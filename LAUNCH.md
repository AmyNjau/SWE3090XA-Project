...
2. In Android Studio, **open the folder** `G:\My Drive\Claude\SWE3090XA\mobile`.
   ...
   > `<queries>` entry, and the Firebase Gradle wiring. Only `local.properties`,
   > `.gradle/`, `.kotlin/` and build output are ignored. If the folder is ever
   > regenerated with `flutter create .`, all of those have to be re-applied —
